...
class PersonalityDataset:
    def __init__(self, path=None, tablesize=1000000, sentences=[]):
        if not path:
            root_dir = Path(__file__).resolve().parents[3]
            path = root_dir / "data" / "clean" / "main_datasets.json"
    ...
    def tokens(self):
        ...
        for sentence in self.sentences():
            ...
            counter += 1
            if counter % 300 == 0:
                print(f"currently on the {counter}th sentence")

        tokens["UNK"] = idx
        revtokens += ["UNK"]
        tokenfreq["UNK"] = 1
        wordcount += 1
        print("getting token is done")
        self._tokens = tokens
